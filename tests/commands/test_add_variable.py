...
def runner(mock_validate_facets_yaml, monkeypatch):
    ...
    monkeypatch.setattr('ftf_cli.utils.ALLOWED_TYPES', ["string", "number", "integer", "boolean", "array", "object", "enum"])
    return CliRunner()
...
def test_add_variable_success(runner):
    ...
    with isfile_patch, open_patch:
        result = runner.invoke(add_variable, ['--name', 'test.name', '--type', 'string', '--description', 'A test variable', '--path', '.'])
...
def test_add_variable_invalid_type(runner):
    ...
    isfile_patch, open_patch = mock_yaml_file_existence_and_content(yaml_content)
    with isfile_patch, open_patch:
        result = runner.invoke(add_variable, ['--name', 'test.name', '--type', 'invalid_type', '--description', 'A test variable', '--path', '.'])
        assert result.exit_code != 0
...
def test_add_enum_variable_with_options(runner):
    ...
    isfile_patch, open_patch = mock_yaml_file_existence_and_content(yaml_content)
    with isfile_patch, open_patch:
        result = runner.invoke(add_variable, ['--name', 'test_enum', '--type', 'enum', '--description', 'An enum test variable', '--options', 'opt1,opt2,opt3', '--path', '.'])
        assert result.exit_code == 0
...
def test_add_enum_variable_without_options(runner):
    ...
    isfile_patch, open_patch = mock_yaml_file_existence_and_content(yaml_content)
    with isfile_patch, open_patch:
        result = runner.invoke(add_variable, ['--name', 'test_enum', '--type', 'enum', '--description', 'An enum test variable', '--path', '.'])
        assert result.exit_code != 0
...
def test_add_variable_empty_spec(runner):
    ...
    with isfile_patch, open_patch:
        ...
        result = runner.invoke(add_variable, ['--name', 'new_variable', '--type', 'string', '--description', 'A new test variable', '--path', '.'])
        assert result.exit_code == 0
        assert "Variable 'new_variable' of type 'string' added with description 'A new test variable' in path '.'" in result.output
