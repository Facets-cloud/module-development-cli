...
def test_add_variable(setup_module_folder):
    ...
    assert 'test_variable' in facets_data['spec']['properties'], "Variable 'test_variable' not found in facets.yaml"
...
def test_validate_directory(setup_module_folder):
    ...
    result = subprocess.run(
        [
            'ftf', 'validate-directory', module_base],
        capture_output=True,
        text=True
    )
    
    assert result.returncode == 0, f"Validation failed: {result.stderr}"
    assert "Terraform validation successful." in result.stdout, "Terraform validation was not successful"
